...
设置是**改一下就自动存**的 —— 方便，但也意味着手滑了没有回头路：
不小心拖了温度、把调了很久的提示词清空、按到「恢复默认提示词」，
原值当时就没了。
...
| | 什么时候存 | 存的是什么 |
|---|---|---|
| 自动快照 | 每次设置变化（防抖 1.5 秒） | **改动之前**的那一份 |
| 手动快照 | 你点「保存快照」 | 当前这一份，可以起名字 |

关键设计：

- **存的是「改动之前」，不是「改动之后」。** 方向反了的话，误触之后回退到的
  是刚改坏的样子，等于没救。
- **恢复之前会自动再存一份当前值。** 所以「恢复」这个动作本身也能撤销 ——
  否则防误触的功能自己就成了最大的误触来源。
- **列表里直接写清楚「这份和现在差在哪」**（`② 温度：0.3 → 1.8`），
  这才是决定要不要恢复的依据。密钥只显示「已设置（sk-a…，35 位）」，
  不把内容打出来。
- 「恢复默认提示词」这类危险操作会**立刻**存一份（不走防抖）。
- 光切页签不产生快照（`ui` 不进快照），否则来回翻页就把格子占满了。
- 最多留 24 份，超了丢最旧的 —— **手动存的优先保留**。

历史存在 `extensionSettings.agent_writer_history`，跟设置同在服务端的
...
history.js         版本历史：自动/手动快照、差异对比、恢复
...
| 脚本 | 项数 | 管什么 |
|---|---|---|
| `smoke.mjs` | 167 | 面板、状态机、菜单项、可见报错、触摸事件、页签接线 |
...
| `test-history.mjs` | 60 | 快照：改动前留得住、恢复回得去、连恢复本身都能撤销 |
